...
module tear_drop_shaft_port(dims, solid=false, interior_only=false) {
  above = GetHeight(dims);
  below = GetDepth(dims);
  h = above + below;
  shaft_id = GetInnerDiam(dims);
  shaft_ir = shaft_id / 2;
  port_od = GetOuterDiam(dims);
  port_or = port_od / 2;
  ...
  module can_ext() {
    ...
    symmetric_z_cylinder(r=helmet_or, l=2*port_od);
  }
  module can_mid() {
    r = (helmet_ir + helmet_or) / 2;
    rotate([0, -90, 0])
    translate([-helmet_or, 0, 0])
    symmetric_z_cylinder(r=r, l=2*port_od);
  }
  module can_int() {
    rotate([0, -90, 0])
    translate([-helmet_or, 0, 0])
    symmetric_z_cylinder(r=helmet_ir, l=2*port_od);
  }
  module can_cutout() {
    intersection() {
      difference() {
...
echo(test_dims);
TDSP_ValidateDims(test_dims);
TDSP_EchoDims(test_dims);
